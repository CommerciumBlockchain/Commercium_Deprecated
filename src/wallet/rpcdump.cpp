...
UniValue importprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "wallet.\n"
            "\nArguments:\n"
            "1. \"commerciumprivkey\"   (string, required) The private key (see "
            "dumpprivkey)\n"
...
UniValue importaddress(const Config &config, const JSONRPCRequest &request) {
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Invalid Commercium address or script");
    }
...
UniValue dumpprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The commercium address for the "
            "private key\n"
            "\nResult:\n"
    ...
    return CCommerciumSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const Config &config, const JSONRPCRequest &request) {
    ...
    // produce output
    file << strprintf("# Wallet dump created by Commercium %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID>>::const_iterator it =
        ...
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CCommerciumSecret(key).ToString(),
                              strTime);
