...
bool ok(CCommerciumAddress a, CCommerciumAddress b)
{
...
isminetype CWallet::IsMine(const CTxOut &txout, std::string addr) const
{
	...
	CKeyID keyID;
	CCommerciumAddress cba;
	const CCommerciumAddress from(addr);
	switch (whichType)
	{
		case TX_PUBKEY:
...
void CWallet::AvailableCoins(std::vector<COutput> &vCoins, bool fOnlyConfirmed,
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
        ...
        // Commercium: Removed check that prevents consideration of coins from
        ...
        // Commercium: retained this check as 'replaced_by_txid' is still set
        // in the wallet code.
...

    {
        ...
        while (true) {
            ...
            if (nChange > 0) {
                ...
                // transaction isn't always pay-to-commercium-address.
