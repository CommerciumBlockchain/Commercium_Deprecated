...
const std::string CommerciumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
    "macosx"
 ...
 */
const QString CommerciumGUI::DEFAULT_WALLET = "~Default";

CommerciumGUI::CommerciumGUI(const PlatformStyle *_platformStyle,
...
void CommerciumGUI::createActions() {
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Commercium address"));
    ...
    signMessageAction->setStatusTip(
        tr("Sign messages with your Commercium addresses to prove you own them"));
    verifyMessageAction =
        new QAction(platformStyle->TextColorIcon(":/icons/verify"),
                    tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(
        tr("Verify messages to ensure they were signed with specified Commercium "
           "addresses"));
    ...
    showHelpMessageAction->setStatusTip(
        tr("Show the %1 help message to get a list with possible Commercium "
...
void CommerciumGUI::createMenuBar() {
#ifdef Q_OS_MAC
...
void CommerciumGUI::createToolBars() {
    if (walletFrame) {
...
void CommerciumGUI::setClientModel(ClientModel *_clientModel) {
...
#ifdef ENABLE_WALLET
bool CommerciumGUI::addWallet(const QString &name, WalletModel *walletModel) {
    if (!walletFrame) return false;
    setWalletActionsEnabled(true);
    return walletFrame->addWallet(name, walletModel);
}

bool CommerciumGUI::setCurrentWallet(const QString &name) {
    if (!walletFrame) return false;
    return walletFrame->setCurrentWallet(name);
}

void CommerciumGUI::removeAllWallets() {
...
void CommerciumGUI::setWalletActionsEnabled(bool enabled) {
...
void CommerciumGUI::createTrayIcon(const NetworkStyle *networkStyle) {
#ifndef Q_OS_MAC
...
void CommerciumGUI::createTrayIconMenu() {
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void CommerciumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason) {
...
void CommerciumGUI::optionsClicked() {
...
void CommerciumGUI::aboutClicked() {
...
void CommerciumGUI::showDebugWindow() {
...
void CommerciumGUI::showDebugWindowActivateConsole() {
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void CommerciumGUI::showHelpMessageClicked() {
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void CommerciumGUI::openClicked() {
...
void CommerciumGUI::gotoOverviewPage() {
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CommerciumGUI::gotoHistoryPage() {
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CommerciumGUI::gotoReceiveCoinsPage() {
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CommerciumGUI::gotoSendCoinsPage(QString addr) {
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CommerciumGUI::gotoSignMessageTab(QString addr) {
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CommerciumGUI::gotoVerifyMessageTab(QString addr) {
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void CommerciumGUI::updateNetworkState() {
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Commercium network", "", count) +
...
void CommerciumGUI::setNumConnections(int count) {
    updateNetworkState();
}

void CommerciumGUI::setNetworkActive(bool networkActive) {
    updateNetworkState();
}

void CommerciumGUI::updateHeadersSyncProgressLabel() {
...
void CommerciumGUI::setNumBlocks(int count, const QDateTime &blockDate,
...
void CommerciumGUI::message(const QString &title, const QString &message,
                         unsigned int style, bool *ret) {
    // default title
    QString strTitle = tr("Commercium");
...
void CommerciumGUI::changeEvent(QEvent *e) {
...
void CommerciumGUI::closeEvent(QCloseEvent *event) {
#ifndef Q_OS_MAC // Ignored on Mac
    if (clientModel && clientModel->getOptionsModel()) {
...
void CommerciumGUI::showEvent(QShowEvent *event) {
...
#ifdef ENABLE_WALLET
void CommerciumGUI::incomingTransaction(const QString &date, int unit,
    ...
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n")
                      .arg(CommerciumUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void CommerciumGUI::dragEnterEvent(QDragEnterEvent *event) {
    // Accept only URIs
    if (event->mimeData()->hasUrls()) event->acceptProposedAction();
}

void CommerciumGUI::dropEvent(QDropEvent *event) {
...
bool CommerciumGUI::eventFilter(QObject *object, QEvent *event) {
...
#ifdef ENABLE_WALLET
bool CommerciumGUI::handlePaymentRequest(const SendCoinsRecipient &recipient) {
    // URI has to be valid
...
void CommerciumGUI::setHDStatus(int hdEnabled) {
...
void CommerciumGUI::setEncryptionStatus(int status) {
    switch (status) {
...
void CommerciumGUI::showNormalIfMinimized(bool fToggleHidden) {
    if (!clientModel) return;
...
void CommerciumGUI::toggleHidden() {
    showNormalIfMinimized(true);
}

void CommerciumGUI::detectShutdown() {
...
void CommerciumGUI::showProgress(const QString &title, int nProgress) {
    if (nProgress == 0) {
...
void CommerciumGUI::setTrayIconVisible(bool fHideTrayIcon) {
    if (trayIcon) {
        trayIcon->setVisible(!fHideTrayIcon);
    }
}

void CommerciumGUI::showModalOverlay() {
    if (modalOverlay &&
        (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CommerciumGUI *gui, const std::string &message,
...
void CommerciumGUI::subscribeToCoreSignals() {
...
void CommerciumGUI::unsubscribeFromCoreSignals() {
...
void CommerciumGUI::toggleNetworkActive() {
    if (clientModel) {
...
UnitDisplayStatusBarControl::UnitDisplayStatusBarControl(
    ...
    QList<CommerciumUnits::Unit> units = CommerciumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CommerciumUnits::Unit unit : units) {
        max_width = qMax(max_width, fm.width(CommerciumUnits::name(unit)));
    }
...
void UnitDisplayStatusBarControl::createContextMenu() {
    menu = new QMenu(this);
    for (CommerciumUnits::Unit u : CommerciumUnits::availableUnits()) {
        QAction *menuAction = new QAction(QString(CommerciumUnits::name(u)), this);
...
void UnitDisplayStatusBarControl::updateDisplayUnit(int newUnits) {
    setText(CommerciumUnits::name(newUnits));
}
