...
#include "config/commercium-config.h" /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update() {
    ...
    QString uri = GUIUtil::formatCommerciumURI(info);
    ...
    if (info.amount)
        html += "<b>" + tr("Amount") + "</b>: " +
                CommerciumUnits::formatHtmlWithUnit(model->getDisplayUnit(),
                                                 info.amount) +
                "<br>";
...
void ReceiveRequestDialog::on_btnCopyURI_clicked() {
    GUIUtil::setClipboard(GUIUtil::formatCommerciumURI(info));
}
