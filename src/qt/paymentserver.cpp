...
const int COMMERCIUM_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
...
void PaymentServer::ipcParseCommandLine(int argc, char *argv[]) {
    for (int i = 1; i < argc; i++) {
                           ...
                           Qt::CaseInsensitive)) // commercium: URI
        {
            ...
            if (GUIUtil::parseCommerciumURI(arg, &r) && !r.address.isEmpty()) {
...
bool PaymentServer::ipcSendCommandLine() {
    ...
    for (const QString &r : savedPaymentRequests) {
        ...
        if (!socket->waitForConnected(COMMERCIUM_IPC_CONNECT_TIMEOUT)) {
            delete socket;
            socket = nullptr;
            return false;
        }
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(COMMERCIUM_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString &s) {
    ...
    if (s.startsWith(GUIUtil::URI_SCHEME + ":", Qt::CaseInsensitive)) {
        ...
        if (uri.hasQueryItem("r")) {
            ...
            if (GUIUtil::parseCommerciumURI(s, &recipient)) {
                ...
                Q_EMIT message(
                    ...
                    tr("URI cannot be parsed! This can be caused by an invalid "
                       "Commercium address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
    ...
    for (const std::pair<CScript, CAmount> &sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom commercium addresses are
            // not supported
        ...
        // Commercium amounts are stored as (optional) uint64 in the protobuf
        ...
        if (txOut.IsDust(dustRelayFee)) {
            Q_EMIT message(
                ...
                tr("Requested payment amount of %1 is too small (considered "
                   "dust).")
                    .arg(CommerciumUnits::formatWithUnit(
