...
#include "commerciumaddressvalidator.h"
...
namespace GUIUtil {
const QString URI_SCHEME("commerciumcash");
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent) {
    ...
    widget->setPlaceholderText(
        QObject::tr("Enter a Commercium address (e.g. %1)")
            .arg(QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new CommerciumAddressEntryValidator(parent));
    widget->setCheckValidator(new CommerciumAddressCheckValidator(parent));
}
...
bool parseCommerciumURI(const QUrl &uri, SendCoinsRecipient *out) {
    // return if URI is not valid or is no commerciumcash: URI
...
bool parseCommerciumURI(QString uri, SendCoinsRecipient *out) {
    // Convert commerciumcash:// to commerciumcash:
    //
    //    Cannot handle this later, because commerciumcash://
    ...
    return parseCommerciumURI(uriInstance, out);
}

QString formatCommerciumURI(const SendCoinsRecipient &info) {
...
static boost::filesystem::path StartupShortcutPath() {
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Commercium.lnk";
    // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Commercium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) /
           strprintf("Commercium (%s).lnk", chain);
}

bool GetStartOnSystemStartup() {
    // check for Commercium*.lnk
...
static boost::filesystem::path GetAutostartFilePath() {
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "commercium.desktop";
    return GetAutostartDir() / strprintf("commercium-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart) {
    if (!fAutoStart)
        ...
        if (!optionFile.good()) return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a commercium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Commercium\n";
        else
            optionFile << strprintf("Name=Commercium (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list,
    ...
    // loop through the list of startup items and try to find the commercium app
...
bool GetStartOnSystemStartup() {
    CFURLRef commerciumAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    ...
    LSSharedFileListItemRef foundItem =
        findStartupItemInList(loginItems, commerciumAppUrl);
    // findStartupItemInList retains the item it returned, need to release
    if (foundItem) {
        CFRelease(foundItem);
    }
    CFRelease(loginItems);
    CFRelease(commerciumAppUrl);
    return foundItem;
}
