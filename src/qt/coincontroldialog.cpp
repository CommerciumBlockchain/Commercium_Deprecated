...
// click on header
    ...
    // see https://github.com/commercium/commercium/issues/5716
...
void CoinControlDialog::copyAmount() {
    GUIUtil::setClipboard(
        CommerciumUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog) {
    ...
    int nDisplayUnit = CommerciumUnits::CMM;
    ...
    // Amount
    l2->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nAmount));
    // Fee
    l3->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nPayFee));
    // After Fee
    l4->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nAfterFee));
    // Bytes
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));
    // Dust
    l7->setText(fDust ? tr("yes") : tr("no"));
    // Change
    l8->setText(CommerciumUnits::formatWithUnit(nDisplayUnit, nChange));
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee)) {
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey,
                ...
                // if listMode or change => show commercium address. In tree mode,
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(
                COLUMN_AMOUNT, CommerciumUnits::format(nDisplayUnit, nSum));
