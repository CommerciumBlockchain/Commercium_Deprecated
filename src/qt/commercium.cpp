...
static std::string Translate(const char *psz) {
    return QCoreApplication::translate("commercium-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase,
    ...
    // Load e.g. commercium_de.qm (shortcut "de" needs to be defined in
    // commercium.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. commercium_de_DE.qm (shortcut "de_DE" needs to be defined in
    // commercium.qrc)
...
/** Class encapsulating Commercium Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class CommerciumCore : public QObject {
    Q_OBJECT
public:
    explicit CommerciumCore();
...
/** Main Commercium application object */
class CommerciumApplication : public QApplication {
    Q_OBJECT
public:
    explicit CommerciumApplication(int &argc, char **argv);
    ~CommerciumApplication();
    ...
    /// Get window identifier of QMainWindow (CommerciumGUI)
...
#include "commercium.moc"

CommerciumCore::CommerciumCore() : QObject() {}

void CommerciumCore::handleRunawayException(const std::exception *e) {
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

void CommerciumCore::initialize(Config *cfg) {
    Config &config(*cfg);
    try {
...
CommerciumApplication::CommerciumApplication(int &argc, char **argv)
    ...
    // This must be done inside the CommerciumApplication constructor, or after it,
    // because PlatformStyle::instantiate requires a QApplication.
    std::string platformName;
    platformName = GetArg("-uiplatform", CommerciumGUI::DEFAULT_UIPLATFORM);
    platformStyle =
...
CommerciumApplication::~CommerciumApplication() {
    if (coreThread) {
...
#ifdef ENABLE_WALLET
void CommerciumApplication::createPaymentServer() {
    paymentServer = new PaymentServer(this);
}
#endif

void CommerciumApplication::createOptionsModel(bool resetSettings) {
    optionsModel = new OptionsModel(nullptr, resetSettings);
}

void CommerciumApplication::createWindow(const NetworkStyle *networkStyle) {
    window = new CommerciumGUI(platformStyle, networkStyle, 0);
...
void CommerciumApplication::createSplashScreen(const NetworkStyle *networkStyle) {
...
void CommerciumApplication::startThread() {
    if (coreThread) return;
    coreThread = new QThread(this);
    CommerciumCore *executor = new CommerciumCore();
...
void CommerciumApplication::parameterSetup() {
    InitLogging();
    InitParameterInteraction();
}

void CommerciumApplication::requestInitialize(Config &config) {
...
void CommerciumApplication::requestShutdown(Config &config) {
...
void CommerciumApplication::initializeResult(int retval) {
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(CommerciumGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(CommerciumGUI::DEFAULT_WALLET);
...
void CommerciumApplication::shutdownResult(int retval) {
    qDebug() << __func__ << ": Shutdown result: " << retval;
    // Exit main loop after shutdown finished.
    quit();
}

void CommerciumApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(
        0, "Runaway exception",
        CommerciumGUI::tr("A fatal error occurred. Commercium can no longer continue "
                       "safely and will quit.") +
            QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId CommerciumApplication::getMainWinId() const {
    if (!window) return 0;

    return window->winId();
}

#ifndef COMMERCIUM_QT_TEST

static void MigrateSettings() {
    assert(!QApplication::applicationName().isEmpty());

    static const QString legacyAppName("Commercium-Qt"),
#ifdef Q_OS_DARWIN
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(commercium);
    Q_INIT_RESOURCE(commercium_locale);

    CommerciumApplication app(argc, argv);
    ...
    // Migrate settings from core's/our old GUI settings to Commercium
    // only if core's exist but Commercium's doesn't.
    ...
    /// 6. Determine availability of data directory and parse commercium.conf
    ...
    try {
        ReadConfigFile(GetArg("-conf", COMMERCIUM_CONF_FILENAME));
    } catch (const std::exception &e) {
        QMessageBox::critical(
            0, QObject::tr(PACKAGE_NAME),
