...
bool Intro::pickDataDirectory() {
    ...
    if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) ||
        ...
        intro.setWindowIcon(QIcon(":icons/commercium"));
     ...
     * to
     * override -datadir in the commercium.conf file in the default data directory
     * (to be consistent with commerciumd behavior)
     */
    if (dataDir != getDefaultDataDirectory())
