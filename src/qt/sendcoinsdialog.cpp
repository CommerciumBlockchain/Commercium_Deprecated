...
void SendCoinsDialog::on_sendButton_clicked() {
    ...
    processSendCoinsReturn(
        prepareStatus,
        CommerciumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
    ...
    for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients()) {
        ...
        QString amount =
            "<b>" + CommerciumUnits::formatHtmlWithUnit(
    ...
    if (txFee > 0) {
        ...
        questionString.append(CommerciumUnits::formatHtmlWithUnit(
    ...
    for (CommerciumUnits::Unit u : CommerciumUnits::availableUnits()) {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(
                CommerciumUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(
        tr("Total Amount %1")
            .arg(CommerciumUnits::formatHtmlWithUnit(
...
void SendCoinsDialog::updateFeeMinimizedLabel() {
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(
            CommerciumUnits::formatWithUnit(
...
void SendCoinsDialog::coinControlChangeEdited(const QString &text) {
    if (model && model->getAddressTableModel()) {
        ...
        if (text.isEmpty()) {
            ...
            // Invalid address
            ui->labelCoinControlChangeLabel->setText(
                tr("Warning: Invalid Commercium address"));
        } else {
            // Valid address
