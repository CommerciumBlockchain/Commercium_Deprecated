        ...
        if (tx.IsCoinBase())
            ...
            CSpentIndexKey spentKey(txin.prevout.hash, txin.prevout.n);
                if (spentInfo.addressType == 1) {
                    in.push_back(Pair("address", CCommerciumAddress(CKeyID(spentInfo.addressHash)).ToString()));
                } else if (spentInfo.addressType == 2)  {
                    in.push_back(Pair("address", CCommerciumAddress(CScriptID(spentInfo.addressHash)).ToString()));
                }
            }
...
static UniValue createrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "key is the commercium address, the numeric value (can be string) is "
            "the " +
            CURRENCY_UNIT +
            " amount\n"
