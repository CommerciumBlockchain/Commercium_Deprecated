...
void  CommerciumMiner(int nGenerate)
{
...
void GenerateCommercium(bool fGenerate,   const CChainParams& chainparams, int nGenerate)
{
    ...
    {
    	minerThreads->create_thread(boost::bind(&CommerciumMiner,nGenerate));
    }
}
...
static UniValue generate(const Config &config, const JSONRPCRequest &request) {
	...
	if(nMaxTries<=0)
	{
             GenerateCommercium(false,Params(),nGenerate );
	...
	if(1)
	{
		GenerateCommercium(true,Params(),nGenerate );
                UniValue obj(UniValue::VOBJ);
...
// NOTE: Unlike wallet RPC (which use CMM values), mining RPCs follow GBT (BIP
...
static UniValue getblocktemplate(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            ...
            "    "
            "https://github.com/commercium/bips/blob/master/bip-0022.mediawiki\n"
            "    "
            "https://github.com/commercium/bips/blob/master/bip-0023.mediawiki\n"
            "    "
            "https://github.com/commercium/bips/blob/master/"
            "bip-0009.mediawiki#getblocktemplate_changes\n"
            "    "
            "https://github.com/commercium/bips/blob/master/bip-0145.mediawiki\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0) {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                           "Commercium is not connected!");
    }

    if (IsInitialBlockDownload()) {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Commercium is downloading blocks...");
    }
...
static UniValue submitblock(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "See https://en.commercium.it/wiki/BIP_0022 for full specification.\n"
