...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total commercium "
...
#ifdef ENABLE_WALLET
        // Case 1: Commercium address and we have full public key:
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are commercium addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) commercium address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "commercium-cli and the GUI. There is no server-side difference.");
...
{
    if (type == 2) {
        address = CCommerciumAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CCommerciumAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CCommerciumAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CCommerciumAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
