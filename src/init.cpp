 ...
 * is shared between commerciumd, and commercium-qt and non-server tools.
 */
...
std::string HelpMessage(HelpMessageMode mode) {
    ...
    strUsage += HelpMessageOpt(
        "-conf=<file>", strprintf(_("Specify configuration file (default: %s)"),
                                  COMMERCIUM_CONF_FILENAME));
    if (mode == HMM_COMMERCIUMD) {
#if HAVE_DECL_DAEMON
        strUsage += HelpMessageOpt(
            "-daemon",
    ...
    strUsage += HelpMessageOpt(
        "-pid=<file>",
        strprintf(_("Specify pid file (default: %s)"), COMMERCIUM_PID_FILENAME));
#endif
    strUsage += HelpMessageOpt(
        "-prune=<n>",
        strprintf(
    ...
    if (mode == HMM_COMMERCIUM_QT) debugCategories += ", qt";
    strUsage += HelpMessageOpt(
        "-debug=<category>",
...
/** Sanity checks
 *  Ensure that Commercium is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly) {
    ...
    // Make sure only a single Commercium process is using the data directory.
...
bool AppInitMain(Config &config, boost::thread_group &threadGroup,
    ...
    LogPrintf("Using config file %s\n",
              GetConfigFile(GetArg("-conf", COMMERCIUM_CONF_FILENAME)).string());
