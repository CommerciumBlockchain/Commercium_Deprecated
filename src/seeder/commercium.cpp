...
#define COMMERCIUM_SEED_NONCE 0x0539a019ca550825ULL
...
class CNode {
    ...
    void PushVersion() {
        ...
        uint64_t nLocalNonce = COMMERCIUM_SEED_NONCE;
        int64_t nLocalServices = 0;
        CAddress me(CService("0.0.0.0"));
        BeginMessage("version");
        int nBestHeight = GetRequireHeight();
        std::string ver = "/commercium-cash-seeder:0.15/";
...
/*
int main(void) {
  CService ip("commercium.sipa.be", 9556, true);
  std::vector<CAddress> vAddr;
  vAddr.clear();
  int ban = 0;
