...
bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/commercium.conf are parsed in qt/commercium.cpp's
    // main()
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) &&
                !boost::algorithm::istarts_with(argv[i], "commercium:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "commerciumd anymore. Use the commercium-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for commerciumd but not for the GUI so do this
        // here
...
int main(int argc, char *argv[]) {
    ...
    // Connect commerciumd signal handlers
    noui_connect();
