...
The RPM spec file provided here is for Commercium-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://commercium.net/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Commercium git repository](https://github.com/commercium/commercium) but are not part of
...
Currently only `Source10` is used, for the example `commercium.conf` file.
...
in the commercium git repository. At present only an SVG version of the Commercium
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the commercium
...
needed when 0.12.1 is released, a proper fix is already in the Commercium git
master branch.
...
idea to build Commercium Core against a static version of that library compiled
...
The `commercium.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `commercium-server` package. At this point, most RPM based
...
The `commercium-server` package should have SELinux support. How to properly do
...
Commercium-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/commercium/). He has been
packaging Commercium for Fedora at least since 2012.
...
4. I renamed the `commercium` package that contains the Qt GUI to `commercium-core` as
that appears to be how the general population refers to it, in contrast to
`commercium-xt` or `commercium-classic`. I wanted to make sure the general population
