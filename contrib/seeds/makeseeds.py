#!/usr/bin/env python3
# Copyright (c) 2013-2017 The Commercium Core developers
...
# CMM/UAHF specification.
PATTERN_AGENT = re.compile(
    r"^(/Commercium:0.15.(\d+)\(\S+\)/|/CommerciumXT:0.11.0G\(\S+\)/|/BUCash:1.1.(\d+)\(\S+\)/|/Classic:1.3.(\d+)\(\S+\)/)")
...
def parseline(line):
    ...
    if len(sline) < 11:
        return None
    # All CMM clients apart BU and Classic has a space in the useragent string
    if len(sline) == 13:
...
def main():
    ...
    # Filter out hosts with multiple commercium ports, these are likely abusive
    ips = filtermultiport(ips)
    # Look up ASNs and limit results, both per ASN and globally.
    # TODO during this bootstrap phase we need any CMM full nodes
    # active on the network, uncomment the following line once the
    # CMM chain will be consolidated.
