#!/bin/bash
# Copyright (c) 2013 The Commercium Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Commercium database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Commercium datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Commercium datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Commercium 0.7 datadir."
    ;;
  3)
    echo "Detected Commercium pre-0.8 datadir."
    ;;
  4)
    echo "Detected Commercium 0.8 datadir."
    ;;
esac
