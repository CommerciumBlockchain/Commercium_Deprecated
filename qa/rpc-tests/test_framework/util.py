...
DEFAULT_COMMERCIUMD = 'commerciumd'
COVERAGE_DIR = None
...
COMMERCIUMD_PROC_WAIT_TIMEOUT = 60
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "commercium.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_commerciumd_start(process, url, i):
    '''
    Wait for commerciumd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if commerciumd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception(
                'commerciumd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir):
    ...
    if create_cache:
        ...
        # Create cache directories, run commerciumds:
        for i in range(MAX_NODES):
            datadir = initialize_datadir(cachedir, i)
            args = [os.getenv("COMMERCIUMD", "commerciumd"), "-server",
                    "-keypool=1", "-datadir=" + datadir, "-discover=0"]
            if i > 0:
                args.append("-connect=127.0.0.1:" + str(p2p_port(0)))
            commerciumd_processes[i] = subprocess.Popen(args)
            logger.debug("Creating data directories from cached datadir")
            wait_for_commerciumd_start(commerciumd_processes[i], rpc_url(i), i)
    ...
    for i in range(num_nodes):
        ...
        # Overwrite port/rpcport in commercium.conf
...
def locate_commerciumd_binary():
    """
    Find commerciumd binary if possible.
    """
    commerciumd_binary = os.getenv("COMMERCIUMD", DEFAULT_COMMERCIUMD)
    if os.path.exists(commerciumd_binary):
        return commerciumd_binary

    if os.path.exists(os.path.join('src', DEFAULT_COMMERCIUMD)):
        commerciumd_binary = os.path.abspath(
            os.path.join('src', DEFAULT_COMMERCIUMD))
    elif commerciumd_binary == 'commerciumd' or not os.path.exists(commerciumd_binary):
        # If COMMERCIUMD was specified and exists, use it, otherwise look for source.
        # get_srcdir() already returns an absolute path
        src_dir_cand = get_srcdir(sys.argv[0])
        if src_dir_cand and os.path.exists(
                os.path.join(src_dir_cand, 'src', DEFAULT_COMMERCIUMD)):
            commerciumd_binary = os.path.join(
                src_dir_cand, 'src', DEFAULT_COMMERCIUMD)
        else:
            sys.stderr.write("Unable to locate commerciumd for this test.\n")
            sys.exit(1)
    return commerciumd_binary
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr_checker=None):
    """
    Start a commerciumd and return RPC connection to it.
    If stderr_checker is provided, it must be an OutputChecker.
    Its output_file_obj will be connected to the stderr of the commerciumd process.
    """
    datadir = os.path.join(dirname, "node" + str(i))
    if binary is None:
        binary = locate_commerciumd_binary()
    ...
    if stderr_checker:
        ...
        commerciumd_processes[i] = subprocess.Popen(args)
    logger.debug(
        "initialize_chain: commerciumd started, waiting for RPC to come up")
    url = rpc_url(i, rpchost)
    wait_for_commerciumd_start(commerciumd_processes[i], url, i)
    logger.debug("initialize_chain: RPC successfully started")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr_checkers=None):
    """
    Start multiple commerciumds, return RPC connections to them
...
def stop_node(node, i):
    ...
    return_code = commerciumd_processes[i].wait(
        timeout=COMMERCIUMD_PROC_WAIT_TIMEOUT)
    assert_equal(return_code, 0)
    del commerciumd_processes[i]


def stop_nodes(nodes):
    for i, node in enumerate(nodes):
        stop_node(node, i)
    assert not commerciumd_processes.values()  # All connections must be gone now
