...
class CommerciumTestFramework(object):
    ...
    def main(self):
        ...
        parser.add_option(
            "--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave commerciumds and test.* datadir on exit or error")
        parser.add_option(
            "--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop commerciumds after the test execution")
        parser.add_option(
            "--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing commerciumd/commercium-cli (default: %default)")
        parser.add_option(
        ...
        if not self.options.noshutdown:
            ...
            self.log.info(
                "Note: commerciumds were not stopped and may still be running")
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as commerciumd's debug.log with microprecision (so
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("CommerciumRPC")
            rpc_logger.setLevel(logging.DEBUG)
            rpc_handler = logging.StreamHandler(sys.stdout)
            rpc_handler.setLevel(logging.DEBUG)
            rpc_logger.addHandler(rpc_handler)

# Test framework for doing p2p comparison testing, which sets up some commerciumd
# binaries:
...
class ComparisonTestFramework(CommerciumTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("COMMERCIUMD", "commerciumd"),
                          help="commerciumd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("COMMERCIUMD", "commerciumd"),
                          help="commerciumd binary to use for reference nodes (if any)")
