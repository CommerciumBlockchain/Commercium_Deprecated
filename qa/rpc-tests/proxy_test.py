#!/usr/bin/env python3
# Copyright (c) 2015-2016 The Commercium developers
...
'''
Test plan:
- Start commerciumd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on commerciumd side:
...
- Create commerciumds that connect to them
- Manipulate the commerciumds using addnode (onetry) an observe effects
...
class ProxyTest(CommerciumTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: commerciumd's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert(isinstance(cmd, Socks5Command))
            # Note: commerciumd's SOCKS5 implementation only sends atyp
        ...
        if test_onion:
            ...
            node.addnode("commerciumostk4e4re.onion:9556", "onetry")
            cmd = proxies[2].queue.get()
            assert(isinstance(cmd, Socks5Command))
            assert_equal(cmd.atyp, AddressType.DOMAINNAME)
            assert_equal(cmd.addr, b"commerciumostk4e4re.onion")
